...
1. **Be valid markdown** — parseable by goldmark (CommonMark + GFM extensions)
2. **Have a clear heading** describing what's being tested (e.g., `# MD009 Trailing Spaces - Frontmatter`)
3. **Contain ONLY violations relevant to the rule being tested** — don't accidentally trigger other rules unless that's the specific test (use clean markdown structure otherwise)
...
3. Not be trivially empty — include enough content to exercise the rule's scanning logic
...
[Violation near emoji] 🎉
[Violation near CJK] 你好世界
...
1. **Read the generated `.golden.md`** — confirm the fix is correct
2. **Read the generated `.diags.json`** — confirm diagnostics are accurate (line, column, message, fixable)
3. **For `clean` cases** — confirm `.golden.md` matches `.input.md` exactly and `.diags.json` is `[]`
4. **For `code_block_immunity` cases** — confirm `.golden.md` matches `.input.md` exactly and `.diags.json` is `[]`
5. **Run round-trip test** — confirm `TestGoldenRoundTrip` passes for the new case
